...
test-server と key-rest デーモンをビルド・起動し、全クレデンシャルを登録し、Python の `key_rest.requests` クライアントライブラリ経由で全 26 サービスをテストするエンドツーエンドテストです。
...
6. `key_rest.requests.get/post` 経由で全 26 サービスに認証付きリクエストを送信し、結果を報告
